...
if(!ML_API_URL) {
  throw new Error("ML_API_URL is not set in environment variables");
    ...
    create table public."AuthorizedTime" (
      ...
      "Name" text null,
      coord jsonb null,
      constraint authorizedtime_pkey primary key (id)
    ) TABLESPACE pg_default;
    */
const REPORT_SCHEMA = {
  type: Type.OBJECT,
...
2. SEVERITY: 
   - 'High': Immediate danger or functional failure, OR human/person detected in image.
   - 'Medium': Functional issue that needs timely intervention, OR energy wastage detected.
   - 'Low': Cosmetic or non-urgent maintenance, OR no issue and no humans detected.
...
HUMAN DETECTION - ALWAYS REPORT:
- ALWAYS check for the presence of any human, person, or living being in the image.
- If you see ANY person/human in the image, ALWAYS report "Unauthorized Person Detected" as the issue.
- This takes priority over infrastructure issues.
- Even if the infrastructure looks fine, if a human is visible, report it with category "Safety".
- Do NOT report "No Issue" if there is a person/human in the image - always report the human.

ENERGY WASTAGE DETECTION:
- ALWAYS check for lights, bulbs, or electrical fixtures that are ON/illuminated.
- If lights or bulbs are visibly ON but no other infrastructure issues are detected:
  - Report issue as "Energy Wastage Detected"
  - Set category to "Electrical"
  - Set severity to "Medium"
  - Use confidence 70-85 depending on clarity
- This applies when there's no broken infrastructure - just unnecessary lighting in unoccupied/daylight areas.
- Examples: Office lights on during daytime, empty corridor lights on, illuminated signs when not needed.

INFRASTRUCTURE ANALYSIS:
- If the image looks normal or clean AND no humans are visible AND no wasteful energy usage:
  - Report "No Issue" with severity "Low" and confidence of 0.
...
Respond ONLY with valid JSON. Focus on campus infrastructure and safety.`;
...
async function checkUnauthorizedHuman(authorizedTimes) {
  ...
  console.log(`[AuthorizedTime Check] Current Time: ${currentTime}`);
  
  // Get current location
  const currentLocation = await getBrowserLocation();
  if (!currentLocation) {
    // If no location, cannot determine authorization
    console.log('[AuthorizedTime Check] No location available');
    return false;
  }
  
  // Check if current time falls within any restricted time range
  for (const timeRange of authorizedTimes) {
    if (!timeRange.TimeMin || !timeRange.TimeMax) continue;
    
    // Ensure TimeMin and TimeMax are strings in HH:MM:SS format
    const timeMin = String(timeRange.TimeMin);
    const timeMax = String(timeRange.TimeMax);
    
    console.log(`[AuthorizedTime Check] DB TimeMin: ${timeMin}, DB TimeMax: ${timeMax}, Current: ${currentTime}`);
    
    // Compare times as strings (HH:MM:SS format enables lexicographic comparison)
    const isInTimeRange = currentTime >= timeMin && currentTime <= timeMax;
    console.log(`[AuthorizedTime Check] Time in range: ${isInTimeRange}`);
    if (!isInTimeRange) continue;
    
    // If time matches, check if location is within the coord boundary
    if (timeRange.coord) {
      try {
        const coordData = typeof timeRange.coord === 'string' 
          ? JSON.parse(timeRange.coord) 
          : timeRange.coord;
        
        // Check if current location is within the restricted zone
        if (coordData.lat && coordData.long && coordData.radius) {
          const distance = haversineDistance(currentLocation, {lat: coordData.lat, long: coordData.long});
          if (distance <= coordData.radius) {
            return true; // Unauthorized human detected in restricted zone during restricted time
          }
        }
      } catch (e) {
        console.warn("Error parsing coord data:", e);
      }
    }
  }
...
async function analyzeImage(base64Data) {
  ...
  try {
    ...
    // Ensure Low priority when no issue is detected
    if (parsed.detectedIssue?.includes('No Issue')) {
      parsed.severityLevel = 'Low';
    }
    
    // Check if any person/human is detected in the image
    const hasPersonDetected = parsed.detectedIssue?.toLowerCase().includes('person') || 
                              parsed.detectedIssue?.toLowerCase().includes('human') ||
                              parsed.category?.toLowerCase().includes('safety') ||
                              parsed.category?.toLowerCase().includes('person');
    
    // If a person is detected, always check authorization
    if (hasPersonDetected) {
      console.log('[Analysis] Person/Human detected in image, checking authorization...');
      const unauthorizedStatus = await checkUnauthorizedHuman(state.authorizedTimes);
      if (unauthorizedStatus) {
        // Override any other issue with unauthorized human detection
        parsed.detectedIssue = 'Unauthorized Human Detected';
        parsed.severityLevel = 'High';
        parsed.category = 'Safety';
        parsed.confidenceLevel = 95;
        console.log('[Analysis] Unauthorized human detected - setting issue to priority');
      }
    }
    // Also check if no visible issue but check for unauthorized humans
    else if (parsed.detectedIssue?.includes('No Issue') && state.authorizedTimes.length > 0) {
      console.log('[Analysis] No visible issue detected, checking for unauthorized humans...');
      const unauthorizedStatus = await checkUnauthorizedHuman(state.authorizedTimes);
      if (unauthorizedStatus) {
        parsed.detectedIssue = 'Unauthorized Human Detected';
        parsed.severityLevel = 'High';
        parsed.category = 'Safety';
        parsed.confidenceLevel = 95;
        console.log('[Analysis] Unauthorized human detected during restricted time');
      }
    }
    // Check for energy wastage (lights/bulbs on)
    if (parsed.detectedIssue?.toLowerCase().includes('energy wastage')) {
      console.log('[Analysis] Energy wastage detected - lights or bulbs are on unnecessarily');
      parsed.category = 'Electrical';
      parsed.severityLevel = 'Medium';
      if (parsed.confidenceLevel < 70) {
        parsed.confidenceLevel = Math.max(parsed.confidenceLevel, 70);
      }
    }
    return parsed;
  } catch(err) {
...
async function autoDispatchClusteredTickets(tickets) {
  console.log('[AutoDispatch] Checking for clustered tickets at similar locations...');
  
  // Filter tickets with location data
  const ticketsWithLocation = tickets.filter(t => t.lat && t.long);
  console.log(`[AutoDispatch] Found ${ticketsWithLocation.length} tickets with location data`);
  
  // Log all tickets with their coordinates
  ticketsWithLocation.forEach(t => {
    console.log(`[AutoDispatch] Ticket ID: ${t.id}, Lat: ${t.lat}, Long: ${t.long}`);
  });
  
  if (ticketsWithLocation.length <= 2) {
    console.log('[AutoDispatch] Not enough tickets (need more than 2) - skipping clustering check');
    return tickets;
  }
  ...
  // Check for tickets in close proximity
  for (const ticket of ticketsWithLocation) {
    if (processedTicketIds.has(ticket.id)) {
      console.log(`[AutoDispatch] Ticket ${ticket.id} already processed, skipping`);
      continue;
    }

    console.log(`\n[AutoDispatch] === Checking proximity for ticket ${ticket.id} at (${ticket.lat}, ${ticket.long}) ===`);

    // Find all tickets within 100 meters of this ticket using lat/long distance calculation
    const nearbyGroup = ticketsWithLocation.filter(otherTicket => {
      const distance = haversineDistance(
        { lat: parseFloat(ticket.lat), long: parseFloat(ticket.long) },
        { lat: parseFloat(otherTicket.lat), long: parseFloat(otherTicket.long) }
      );
      const distanceInMeters = distance * 1000;
      const isNearby = distance < 0.10; // 100 meters radius in km
      
      console.log(`[AutoDispatch] Ticket ${otherTicket.id}: distance = ${distanceInMeters.toFixed(1)}m, isNearby = ${isNearby}`);
      
      return isNearby;
    });

    console.log(`[AutoDispatch] Total nearby group size for ticket ${ticket.id}: ${nearbyGroup.length} tickets`);

    // If group has more than 2 tickets, auto-dispatch them
    if (nearbyGroup.length > 2) {
      console.log(`[AutoDispatch] ✓ CLUSTER FOUND! ${nearbyGroup.length} tickets at location (${ticket.lat}, ${ticket.long})`);
      nearbyGroup.forEach(t => {
        if (!processedTicketIds.has(t.id)) {
          console.log(`[AutoDispatch] - Adding ticket ${t.id} to dispatch queue`);
          ticketsToDispatch.push(t);
          processedTicketIds.add(t.id);
        }
      });
    } else {
      console.log(`[AutoDispatch] ✗ No cluster for ticket ${ticket.id} (only ${nearbyGroup.length} nearby, need >2)`);
    }
  }

  // Execute auto-dispatch if clusters found
  if (ticketsToDispatch.length > 0) {
    console.log(`\n[AutoDispatch] ========== AUTO-DISPATCHING ${ticketsToDispatch.length} CLUSTERED TICKETS ==========`);
    alert(`Found ${ticketsToDispatch.length} tickets in close proximity. Auto-dispatching now.`);
    
    // Set state.reports so dispatchTicket can find the tickets
    state.reports = tickets;

    // Dispatch all clustered tickets
    const dispatchPromises = ticketsToDispatch.map(t => window.dispatchTicket(t.id, false));
    await Promise.all(dispatchPromises);

    // Remove dispatched tickets from the list
    const dispatchedIds = new Set(ticketsToDispatch.map(t => t.id));
    return tickets.filter(t => !dispatchedIds.has(t.id));
  }

  console.log('[AutoDispatch] No clusters found - all tickets have less than 3 in proximity');
  return tickets;
}
...
async function fetchAndSetTickets() {
  ...
  if (data) {
    ...
    // Use the new auto-dispatch function to check for and dispatch clustered tickets
    reports = await autoDispatchClusteredTickets(reports);
